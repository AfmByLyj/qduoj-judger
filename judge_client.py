...
class JudgeClient(object):
    def __init__(self, run_config, exe_path, max_cpu_time, max_memory, test_case_dir,
        ...
        self._max_real_time = self._max_cpu_time * 10
    ...
    #
    def _get_test_case_sys_out(self, test_case_file_id):
        with open(os.path.join(self._test_case_dir, "{}.out".format(test_case_file_id)), "rb") as f:
            return f.read().decode("utf-8")
    
    def _get_test_case_sys_in(self, test_case_file_id):
        with open(os.path.join(self._test_case_dir, "{}.in".format(test_case_file_id)), "rb") as f:
            return f.read().decode("utf-8")

    def _compare_us(self, user, sys):
        if(len(user) != len(sys)): return False
        for q, p in zip(user, sys):
            q = q.strip()
            p = p.strip()
            if q != p: return False
        return True
    ...
    def _compare_output(self, test_case_file_id, user_output_file):
        ...
        #
        user_out = content.decode("utf-8").strip().split("\n")
        sys_put = self._get_test_case_sys_out(test_case_file_id).strip().split("\n")

        result = self._compare_us(user_out, sys_put)

        # in_sys, out_sys = self._get_test_case_file_info(test_case_file_id)["input_sys"], self._get_test_case_file_info(test_case_file_id)["output_sys"]
        return output_md5, result
    
    def _user_output(self, user_output_file):
        with open(user_output_file, "rb") as f:
            content = f.read()
    ...
    def _spj(self, in_file_path, user_out_file_path):
        ...
        result = _judger.run(max_cpu_time=self._max_cpu_time * 3,
                             max_real_time=self._max_cpu_time * 10,
    ...
    def _judge_one(self, test_case_file_id):
        ...
        run_result["input_sys"] = None
        run_result["output_user"] = None
        if run_result["result"] == _judger.RESULT_SUCCESS:
            if not os.path.exists(user_output_file):
                ...
                if len(run_result['output_user']) >= 10000: run_result['output_user'] = run_result['output_user'][:9997] + "..."
        if self._output:
            try:
                with open(user_output_file, "rb") as f:
